# 08-LOOP - Control de Iteraciones del Workflow de Hacking

> Este archivo es opcional y sirve para proyectos que requieran múltiples
> iteraciones de pentesting. Para auditorías simples, puede eliminarse.

---

## Configuración

```yaml
ITERACIONES_TOTALES: 3
ITERACION_ACTUAL: 1
FECHA_INICIO: "2026-06-05"
ULTIMA_ACTUALIZACION: "2026-06-05"
ESTADO: "EN_PROGRESO"
```

---

## Objetivo

Ejecutar múltiples iteraciones del workflow de hacking para completar todas
las fases de pentesting de manera sistemática.

---

## Flujo del Loop

```
AGENTS.md → FASE 1 (RECON) → FASE 2 (SCAN) → FASE 3 (EXPLOIT) →
FASE 4 (PERSIST) → FASE 5 (CLEANUP) → QA-BROWSER → 08-LOOP.md
                    ↓
              Revisar progreso
                    ↓
              Actualizar contador
                    ↓
              Si ITERACION_ACTUAL < ITERACIONES_TOTALES → Continuar
              Si ITERACION_ACTUAL = ITERACIONES_TOTALES → Finalizar
```

---

## Iteraciones

### Iteración 1 (En progreso)
| Campo | Valor |
|-------|-------|
| Inicio | 2026-06-05 |
| Estado | ⚠️ EN_PROGRESO |
| Fases completadas | Setup inicial |

### Iteración 2 (Pendiente)
| Campo | Valor |
|-------|-------|
| Inicio | - |
| Estado | ⏳ PENDIENTE |
| Tareas | Pentesting completo, explotación |

### Iteración 3 (Pendiente)
| Campo | Valor |
|-------|-------|
| Inicio | - |
| Estado | ⏳ PENDIENTE |
| Tareas | Informe final, remediación |

---

## Cómo Usar Este Archivo

### Al Iniciar una Iteración
1. Leer `AGENTS.md` para contexto del workflow
2. Revisar `feature_list.json` para ver tareas pendientes
3. Ejecutar las fases en orden: RECON → SCAN → EXPLOIT → PERSIST → CLEANUP

### Al Finalizar una Fase
1. Actualizar este archivo con el progreso
2. Marcar tareas completadas en `feature_list.json`
3. Continuar con la siguiente fase

### Al Llegar a Este Archivo (08-LOOP.md)
1. **Revisar progreso**: ¿Qué fases se completaron?
2. **Actualizar contador**: `ITERACION_ACTUAL += 1`
3. **Si hay más iteraciones**: Volver a `AGENTS.md`
4. **Si se completaron todas**: Finalizar y documentar resultados

---

## Comandos de Control

### Iniciar Iteración
```markdown
# ITERACION_ACTUAL: 2
# ULTIMA_ACTUALIZACION: [FECHA_HOY]
```

### Completar Iteración
```markdown
# ITERACION_ACTUAL: 2 → 3
# Completado: [FASES_REALIZADAS]
```

### Finalizar Workflow
```markdown
# ESTADO: COMPLETADO
# FECHA_FIN: [FECHA_HOY]
```

---

## Tareas del feature_list.json

Ver archivo `feature_list.json` para ver el listado completo de tareas.

### Pendientes de Esta Iteración
- [ ] [FASE 1: Reconocimiento]
- [ ] [FASE 2: Escaneo]
- [ ] [FASE 3: Explotación]
- [ ] [FASE 4: Persistencia]
- [ ] [FASE 5: Limpieza]

---

## Referencias

| Archivo | Propósito |
|---------|-----------|
| `AGENTS.md` | Punto de entrada y contexto |
| `feature_list.json` | Lista de tareas |
| `TASK-PRINCIPAL.md` | Objetivo global |
| `tasks/*.md` | Tareas por agente/fase |
| `progress/` | Seguimiento de sesiones |

---

*08-LOOP - Control de Iteraciones - Hacking Harness*
